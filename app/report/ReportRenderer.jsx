...
export default function ReportRenderer({ data, reportId, sample = false }) {
  ...
  // One resolver for all three outputs (screen, PDF, .docx) — see
  // resolveSectionFlags in lib/reportShared.js for the include/exclude history.
            ...
            <div className="cover-footer">
              ...
              <p style={{ margin: '0 0 40px', fontSize: '10px', color: NAVY_LT, letterSpacing: '1.5px', textTransform: 'uppercase' }}>
                {reportId
                  ...
                  : sample ? 'Sample — fictional project' : 'Draft — not saved'}
              </p>
              {/* Bottom accent line */}
          ...
          <div style={{ padding: '32px 56px 48px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '32px' }}>
              ...
              <InfoBox label="Programme"
                ...
                note={[programme?.floatWeeks > 0 ? `Incl. ${programme.floatWeeks} wks float (best case ${programme.totalWeeksBestCase} wks)` : null, programme?.targetNote].filter(Boolean).join('  |  ')}
            ...
            <p style={{ ...bodyText, marginBottom: '8px' }}>
              <strong style={{ color: NAVY }}>Total programme:</strong> {programmeHeadline(programme)}
            ...
            {programme?.stages?.length > 0 && (
              <GanttBar stages={programme.stages} totalWeeks={programme.totalWeeks} surveyWeeks={programme.surveyWeeks}
                startDate={programme.startDate} endDate={programme.endDate} startDateAssumed={programme.startDateAssumed} />
            )}
            ...
            {programme?.fastTrackOptions?.length > 0 && <>
              <SubHdr>Programme Acceleration Options</SubHdr>
              <p style={{ ...bodyText, fontStyle: 'italic', color: '#666', marginBottom: '6px' }}>
                The headline programme does not assume any of these. Each is a lever from the Programme workbook whose trigger applies to this project, with the trade-off the client would accept to use it.
              </p>
              <ul style={listStyle}>
                {fastTrackLines(programme).map((l, i) => <li key={i} style={liStyle}>{l}</li>)}
              </ul>
            </>}
            ...
            <p style={{ ...bodyText, fontSize: '13px', marginBottom: '12px' }}>
              <strong style={{ color: NAVY }}>{areaLabel(cost?.projectType || answers?.q1_2_projectType)}:</strong> {cost?.gifa} m²
              <span style={{ color: GRAY, margin: '0 8px' }}>|</span>
            ...
            {cost?.trace && (
              <div>
                <SubHdr>How each percentage was derived</SubHdr>
                <p style={{ ...bodyText, fontStyle: 'italic', color: '#666', marginBottom: '6px' }}>
                  Every percentage above is the sum of the NRM1 workbook rules listed here, each triggered by a questionnaire answer. Nothing is applied that is not shown.
                </p>
                <BuildUpTable cost={cost} />
              </div>
            )}
            ...
            {/* Scenario comparison — screen only. Deterministic re-runs of the
                same answers with one input varied (POST /api/compare), so it
                costs nothing and cannot disagree with the tables above. Not
                in the PDF or .docx: it is a what-if tool, not part of the
                report of record. */}
            ...
            {showProc && (
              <>
                ...
                <p style={{ ...bodyText, marginBottom: '8px' }}>
                  ...
                  <span style={{ color: GRAY, margin: '0 8px' }}>|</span>
                  <strong style={{ color: NAVY }}>Contract:</strong> {aiProse?.procurementContractForm || programme?.contractForm}
                </p>
                <p style={{ ...bodyText, marginBottom: '16px' }}>
                  <strong style={{ color: NAVY }}>Design responsibility:</strong> {aiProse?.procurementDesignResp || programme?.designResponsibility}
                  <span style={{ color: GRAY, margin: '0 8px' }}>|</span>
                  <strong style={{ color: NAVY }}>Tender type:</strong> {aiProse?.procurementTenderType || programme?.tenderType}
                  {programme?.procurementSource && <>
                    <span style={{ color: GRAY, margin: '0 8px' }}>|</span>
                    <span style={{ color: '#666', fontSize: '12px' }}>Selected by: {programme.procurementSource}</span>
                  </>}
                </p>
...
function TotalCostTable({ cost }) {
  ...
  rows.push(
    ...
    [`VAT @ ${vatPct}% (reference — recoverability to be confirmed)`, `${vatPct}%`, tL*vatPct/100, tH*vatPct/100, false, true ],
  )
...
// Mirrors buildUpTable in lib/reportBuilder.js — both read the same rows from
// lib/reportShared.js's buildUpRows(), so only the presentation lives here.
  ...
  return (
    <div style={{ overflowX: 'auto', marginBottom: '16px' }}>
      <table style={tblStyle}>
        <thead>
          <tr>
            <th style={{ ...thStyle, textAlign: 'left', width: 48 }}>Code</th>
            <th style={{ ...thStyle, textAlign: 'left' }}>Workbook rule / trigger</th>
            <th style={{ ...thStyle, textAlign: 'right', width: 80 }}>%</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} style={{ background: r.subtotal ? ALT_ROW : '#fff', borderBottom: `1px solid ${BORDER}` }}>
              <td style={{ ...tdStyle, color: '#666' }}>{r.subtotal ? '' : r.code}</td>
              <td style={{ ...tdStyle, fontWeight: r.subtotal ? 700 : 400, color: r.subtotal ? NAVY : '#333' }}>{r.label}</td>
              <td style={{ ...tdStyle, textAlign: 'right', fontWeight: r.subtotal ? 700 : 400, color: r.subtotal ? NAVY : '#333' }}>
                {r.subtotal ? fmtPlainPct(r.pct) : fmtSignedPct(r.pct)}
              </td>
            </tr>
          ))}
        </tbody>
          ...
          {stages.map((s, i) => {
            ...
            return (
              <tr key={i} style={{ background: i % 2 === 0 ? ALT_ROW : '#fff', borderBottom: `1px solid ${BORDER}` }}>
                ...
                <td style={tdStyle}>
                  {s.activity}
                  {stageDateRange(s) && <span style={{ display: 'block', fontSize: '11px', color: GRAY, marginTop: 2 }}>{stageDateRange(s)}</span>}
                </td>
...
function GanttBar({ stages, totalWeeks, surveyWeeks, startDate, endDate, startDateAssumed }) {
  ...
  return (
    <div className="avoid-break" style={{ margin: '14px 0 16px' }}>
      <p style={{ fontWeight: 600, fontSize: '11px', color: GRAY, textTransform: 'uppercase', letterSpacing: '0.6px', margin: '0 0 5px', display: 'flex', justifyContent: 'space-between' }}>
        <span>Indicative Programme Overview</span>
        {startDate && endDate && <span style={{ textTransform: 'none', letterSpacing: 0, fontWeight: 500 }}>{fmtDate(startDate)} → {fmtDate(endDate)}{startDateAssumed ? ' (start assumed)' : ''}</span>}
      </p>
...
// The range, base-date and rate-source sentences come from lib/reportShared.js
// so neither renderer can word them differently.
