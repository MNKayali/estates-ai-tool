...
const INCLUDED = [
  { k: '01', t: 'Order of Cost Estimate', d: 'Deterministic NRM1 build-up — elemental rates, BCIS location factor, prelims, fees, risk and inflation. No figure is ever invented.' },
  { k: '02', t: 'RIBA Programme', d: 'Stage 2–4 design, surveys, planning and construction durations, size-banded and modifier-adjusted, with a Gantt overview.' },
  { k: '03', t: 'Risk Register', d: 'A deterministic seed set of project risks, RAG-rated, with mitigations written in plain English.' },
  { k: '04', t: 'Procurement Route', d: 'A recommended route and tendering approach matched to value, programme and complexity.' },
]
    ...
    <div>
        ...
        <div style={wrap(1120)} >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0' }}>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <Link href="/preview/styleguide" className="btn btn-ghost hide-sm">Style guide</Link>
              <Link href="/preview/access" className="btn btn-primary">Access ▸</Link>
            </div>
          </div>
        </div>
      </header>
        ...
        <div style={{ ...wrap(1120), position: 'relative' }}>
          <div style={{ padding: '92px 0 88px', maxWidth: 760 }}>
            ...
            <p className="rise rise-3" style={{ fontSize: 19, lineHeight: 1.6, color: '#AFC2DE', maxWidth: 560, margin: '22px 0 0' }}>
              Deterministic NRM1 cost and RIBA programme, generated on demand — a costed,
              programmed Stage&nbsp;1 report in minutes, not weeks.
            </p>
            <div className="rise rise-4" style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 34 }}>
              <Link href="/preview/access" className="btn btn-primary" style={{ fontSize: 16, padding: '14px 26px' }}>Start a report ▸</Link>
              <Link href="/preview/styleguide" className="btn btn-on-dark" style={{ fontSize: 16, padding: '14px 26px' }}>See the system</Link>
            </div>
            <p className="rise rise-4 mono" style={{ marginTop: 18, fontSize: 12, color: '#7E92B4', letterSpacing: '.04em' }}>
              ~10–15 MIN · ON-SCREEN + .DOCX + PDF EXPORT
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ ...wrap(1120), padding: '40px 0 80px' }}>
        <Card className="panel-dark" style={{ padding: '48px 44px', border: 'none', display: 'flex',
          ...
          <Link href="/preview/access" className="btn btn-primary" style={{ fontSize: 16, padding: '14px 28px' }}>Begin assessment ▸</Link>
        </Card>
      </section>
...
function Brand({ light = false }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <span className="display" style={{ fontWeight: 700, fontSize: 17, color: light ? '#fff' : 'var(--ink)' }}>Estates AI</span>
    </div>
  )
}
  ...
  return (
    <footer style={{ borderTop: '1px solid var(--border)', background: 'rgba(255,255,255,.5)' }}>
      <div style={{ ...wrap(1120), padding: '26px 0', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <span className="mono" style={{ fontSize: 12, color: 'var(--text-mute)' }}>ESTATES AI · DESIGN PREVIEW</span>
        <span style={{ fontSize: 12, color: 'var(--text-mute)' }}>Indicative planning tool — not a substitute for professional advice.</span>
      </div>
    </footer>
  )
}
