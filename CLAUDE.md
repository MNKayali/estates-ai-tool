...
- **Q3.3 surveys: blank means "None".** The question is optional. `computeConfidence()` has always treated an empty answer as no surveys; `checkCondition()` now does too, so a report can no longer say "Grade C, no surveys commissioned" while pricing the project as though surveys were in hand. Any scenario that leaves surveys blank carries the workbook's +2pp risk (and +0.5pp developer costs where a planning route applies).
