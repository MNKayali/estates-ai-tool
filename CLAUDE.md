...
- Optional Durations rows **`SV7`** (ecology/bat survey — Q3.8 ecology option, or roof scope `2.3/2.16/7.2/2.11` on a pre-2000 building; parallel) and **`BS1`** (Building Safety Regulator Gateway 2 — Q3.8 higher-risk building; sequential between Stage 4/BC and tender, `bsaGatewayWeeks`). No row → no stage, no duration: the code never invents one.
...
- **AI scope suggestion:** `POST /api/suggest-scope { objective, projectType, buildingUse, interventionLevel }` — Haiku with a strict tool whose `code` enum is exactly the codes the picker would offer (same visible-group / building-use / tier / `priceable` filters, which are duplicated in the route and must stay in step with the questionnaire). The model proposes; the user edits; the engine prices. `ScopeSuggestBar` in the questionnaire applies the result through the same path as the preset. Protected + rate-limited (20/10m).
...
**September 2026 assessment pass (second round).** A full reassessment of the app, questions and scope picker was written up and then implemented as five phases at £0 (see the plan's Context section for the list). Everything below is done, built, linted, unit-tested (89 Vitest cases, 2 skipping until the workbook is updated) and baselined (`scripts/__baseline__/before-phase1` → `after-phase4`: only the intended float/inflation deltas moved; phases 2–4 IDENTICAL):
...
- *UK checks:* Q3.8 (conservation area, party wall, higher-risk building, ecology) with fixed-rating risk seeds `CTX-*`, Tab 3 `higher-risk` rules, `BS1` Gateway 2 stage, `SV7` ecology survey; Q4.3a VAT position in the budget verdict; storeys to 7+ with an HRB hint.
