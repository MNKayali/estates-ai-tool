  ...
  const priceLine = (item, option, qty, qtySource, estimate, usesRatio = false) => {
    ...
    if (!(qty > 0)) {
      excludedNoQuantity.push({
        ...
        // No invented cause: the old "(e.g. no upper floors on a single-storey
        // building)" example was repeated by the AI as a fact about buildings
        // that had upper floors, and about external works with no building at
        // all (batch-2 OM-6, EW-4, EW-6).
        reason: qtySource === 'user' ? 'your quantity is zero' : 'the estimate from the answers given comes to zero — enter the real quantity in "I know this" if this item is needed',
      })
      return null
    }
