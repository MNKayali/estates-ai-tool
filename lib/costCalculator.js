...
// Pricing types whose quantity is an area (m²) and therefore carry the Q2.2
...
export async function calculateCost(answers, programmeWeeks, constructionWeeks = 0, opts = {}) {
  ...
  // rate by the Q2.2 band factor, so a fabric-only strip costs less than a full
  ...
  for (const code of pricedCodes) {
    ...
    // Band (Q2.2 level of works) applies only to GIFA-rated items; BCIS applies
