...
function parseSpecLevelMap(wb) {
  ...
  for (const row of rows) {
    ...
    if (!knownLevels.has(label)) continue
...
function getQuantity(code, unit, gifa, specialQtys) {
  switch (unit) {
    ...
    case 'Nr':
      if (code === '4.2')  return specialQtys.bathroomNr || Math.max(1, Math.round(gifa / 20))
      ...
      if (code === '8.9')  return specialQtys.extLightNr || Math.max(1, Math.round(gifa / 25))
...
function evaluatePercentageRules(rules, answers, worksTotal, programmeWeeks) {
  ...
  // ── Utility constraint uplifts (Q4.6) ─────────────────────────────────────
  // Adds risk when utility capacity is limited or unknown.
  // Values: electrical +1% (highest impact), all others +0.5%.
  // Note: add corresponding rows to Tab 3 of the NRM1 workbook to make these
  // editable without a code change.
  const utilities = answers.q4_8_utilities || []
  if (utilities.some(u => /electrical/i.test(u)))  eTotal += 1.0
  if (utilities.some(u => /gas\s+supply/i.test(u))) eTotal += 0.5
  if (utilities.some(u => /drainage/i.test(u)))     eTotal += 0.5
  if (utilities.some(u => /water\s+supply/i.test(u))) eTotal += 0.5
...
export async function calculateCost(answers, programmeWeeks) {
  ...
  const specialQtys = {
    ...
    extLightNr:  Number(answers.q2_5_extLightNr) || 0,
