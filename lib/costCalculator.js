...
function getResidentialHiddenItems(subType) {
  const st = (subType || 'flat or house').toLowerCase()
  if (st.includes('student') || st.includes('pbsa')) {
    return ['5.3', '5.14', '5.13']
  }
  if (st.includes('care') || st.includes('sheltered')) {
    return ['5.13', '5.14']
  }
  // Default: flat or house / HMO
  return ['5.9b', '5.7a', '5.16', '5.3', '5.14', '5.13', '5.18']
}
...
function evaluatePercentageRules(rules, answers, worksTotal, programmeWeeks) {
  ...
  // fallback ladder fires only when checkCondition() finds no matching row.
  let cTotal
  if (cBase) {
...
export async function calculateCost(answers, programmeWeeks) {
  ...
  // ── Residential sub-type filter ───────────────────────────────────────────
  // Applied BEFORE the BWIC step so hidden M&E items cannot trigger a phantom
  // Builder's Work in Connection (5.20) line with no surviving parent element.
  const bu = (answers.q1_3_buildingUse || '').toLowerCase()
  const isResidentialType = bu.includes('residential') || bu.includes('student accommodation') || bu.includes('pbsa')
  const effectiveSubType = (bu.includes('student') || bu.includes('pbsa'))
    ? 'Student accommodation'
    : answers.q1_3a_residentialSubType
  const residentialHidden = isResidentialType
    ? getResidentialHiddenItems(effectiveSubType)
    : []
  const isHiddenBySubType = code =>
    residentialHidden.some(h => code === h || code.startsWith(h + '.'))
  for (const c of [...elementCodes]) {
    if (isHiddenBySubType(c)) elementCodes.delete(c)
  }
