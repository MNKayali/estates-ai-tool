...
function getBandFactor(interventionLevel) {
  ...
  if (lv.includes('strip')) return 1.095
  return 1.0 // full refurbishment
}

function getQuantity(code, unit, gifa, specialQtys) {
  switch (unit) {
    case 'm²': return gifa
    case 'Item': return gifa
    case 'Nr':
      if (code === '5.19') return specialQtys.liftNr || 0
      if (code === '5.15') return specialQtys.evNr || 0
      if (code === '4.2') return specialQtys.sanitNr || Math.max(1, Math.round(gifa / 20))
...
export async function calculateCost(answers, programmeWeeks) {
  ...
  const specialQtys = {
    ...
    sanitNr: Number(answers.q2_5_sanitNr) || 0,
  }
  ...
  for (const item of scopeItems) {
    ...
    if (codes) codes.forEach(c => elementCodes.add(c))
  }
  ...
  for (const code of finalCodes) {
    ...
    const midRate = baseRate * bcisFactor * bandFactor
    ...
    lineItems.push({
      ...
      rateLow: Math.round(baseRate * bcisFactor * 0.89 * 100) / 100,
      rateHigh: Math.round(baseRate * bcisFactor * 1.11 * 100) / 100,
