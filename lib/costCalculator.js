...
function getBandFactor(interventionLevel) {
  if (!interventionLevel) return 1.0
  const lv = interventionLevel.toLowerCase()
  if (lv.includes('like-for-like')) return 1.0  // item rates only — band irrelevant
  if (lv.includes('light')) return 0.89
  if (lv.includes('strip')) return 1.11
  return 1.0 // Refurbishment
}
...
function getQuantity(code, unit, gifa, specialQtys) {
  switch (unit) {
    case 'm²': return gifa
    case 'Item':
      if (code === '5.2L') return 1   // fixed qty=1 regardless of GIFA
      return gifa
    case 'Nr':
      if (code === '5.19') return specialQtys.liftNr || 0
      if (code === '5.15') return specialQtys.evNr || 0
      if (code === '4.2') return specialQtys.bathroomNr || specialQtys.sanitNr || Math.max(1, Math.round(gifa / 20))
      if (code === '4.3') return specialQtys.kitchenNr || Math.max(1, Math.round(gifa / 100))
      if (code === '8.3') return specialQtys.carParksNr || Math.max(1, Math.round(gifa / 30))
      return gifa
...
function isGroup0Triggered(scopeItems, knownIssues, interventionLevel) {
  ...
  const isStripOut = (interventionLevel || '').toLowerCase().includes('strip')
  return hasDemo || hasAsbestos || isStripOut
}
...
function evaluatePercentageRules(rules, answers, worksTotal, programmeWeeks) {
  ...
  function checkCondition(condition) {
    ...
    if (c.includes('full planning + listed') || c.includes('listed building consent'))
      return (answers.q3_4_planningConsents || '').toLowerCase().includes('listed')
    if (c.includes('full planning') && !c.includes('listed'))
      return (answers.q3_4_planningConsents || '') === 'Full planning'
    if (c.includes('change of use'))
      return (answers.q3_4_planningConsents || '').toLowerCase().includes('change of use')
    if (c.includes('prior approval'))
      return (answers.q3_4_planningConsents || '').toLowerCase().includes('prior approval')
    if (c.includes('permitted development') && !c.includes('no consent'))
      return (answers.q3_4_planningConsents || '').toLowerCase().includes('permitted development')
    if (c.includes('unsure') && c.includes('pre-application'))
      return (answers.q3_4_planningConsents || '').toLowerCase().includes('unsure')
    if (c.includes('q3.3 = no surveys') || c.includes('no surveys yet'))
      return (answers.q3_3_surveys || '') === 'None yet'
    ...
    if (c.includes('stage 0') || c.includes('stage 0–1'))
      return (answers.q4_6_designStage || '').includes('0')
    if (c.includes('stage 2') && !c.includes('stage 2–4') && !c.includes('0'))
      return (answers.q4_6_designStage || '') === 'Stage 2'
    if (c.includes('stage 3'))
      return (answers.q4_6_designStage || '') === 'Stage 3'
    if (c.includes('stage 4'))
      return (answers.q4_6_designStage || '') === 'Stage 4'
...
export async function calculateCost(answers, programmeWeeks) {
  ...
  const interventionLevel = answers.q2_3_interventionLevel || 'Refurbishment'
  const isLikeForLike = interventionLevel === 'Like-for-like replacement'
  const scopeItems = answers.q2_2_scopeItems || []
  const knownIssues = answers.q3_1_knownIssues || []

  const { factor: bcisFactor, region: bcisRegion } = getBcis(answers.q1_1_postcode, bcisRegions)
  const bandFactor = getBandFactor(interventionLevel)

  // Special quantities from questionnaire
  const specialQtys = {
    pvKwp:      Number(answers.q2_5_pvKwp) || 0,
    battKwh:    Number(answers.q2_5_battKwh) || 0,
    evNr:       Number(answers.q2_5_evNr) || 0,
    liftNr:     Number(answers.q2_5_liftNr) || 0,
    carParksNr: Number(answers.q2_5_carParksNr) || 0,
    sanitNr:    Number(answers.q2_5_sanitNr) || 0,  // legacy
    bathroomNr: Number(answers.q2_2_bathrooms) || 0, // v4 field
    kitchenNr:  Number(answers.q2_2_kitchens) || 0,  // v4 field
  }
  ...
  for (const item of scopeItems) {
    ...
    if (codes) {
      ...
      // Item is a direct NRM1 code (new granular elements bypass Tab 7)
  ...
  // Map new text-label scope items that store NRM1 codes via separate fields
  const labelToCode = {
    'Sub-distribution and containment':              '5.7a',
    '2nd fix lighting':                              '5.8c',
    'Fire alarm system':                             '5.9a',
    'Emergency lighting':                            '5.9b',
    'Gas boiler or plant — like-for-like replacement': '5.2L',
  }
  for (const item of scopeItems) {
    if (labelToCode[item]) elementCodes.add(labelToCode[item])
  }

  // Always include Group 3 (internal finishes) — unless Like-for-like
  if (!isLikeForLike) {
  ...
  for (const code of finalCodes) {
    ...
    if (!el) continue
    // Like-for-like: skip all m² elements — only price Item and Nr rates
    if (isLikeForLike && el.unit === 'm²') continue
  ...
  const worksLow = r1k(worksTotal * 0.89)
  const worksHigh = r1k(worksTotal * 1.11)
...
// Export parser for the health-check endpoint
export async function getRatesWorkbookInfo() {
  const wb = await fetchRatesWorkbook()
  const elements = parseRatesTable(wb)
  const rules = parsePercentageRules(wb)
  const bcis = parseBcisFactors(wb)
  const sampleEl = elements['3.2']
  return {
    ratesRows: Object.keys(elements).length,
    ruleRows: rules.length,
    bcisRegions: bcis.length,
    sampleRate: sampleEl ? { element: sampleEl.description, rfbStd: sampleEl.rfbStd } : null,
