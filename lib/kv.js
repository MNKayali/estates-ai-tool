 ...
 * deletes it; one without — a free-trial visitor's (`anonId`) — keeps the
 * 90-day expiry (7,776,000 seconds) until an account claims it (claimReport).
 *
...
import { isoWeek } from './usageStats.js'
...
export async function createReport(id, data) {
  try {
    ...
    // Listed as "In progress" on My reports straight away; finalise replaces
    // the summary. Best effort — a missing summary only hides it until then.
    if (data?.ownerId) {
      await kv.hset(userReportsKey(data.ownerId), { [id]: reportSummary(id, data, 'in-progress') })
        .catch(e => console.warn('[KV] in-progress summary skipped:', e.message))
    }
    return true
  } catch (e) {
...
export async function finaliseReport(id, data) {
  try {
    // Who the report belongs to comes from the stored record, not the caller:
    // a free-trial report may have been claimed by an account (claimReport)
    // after the prose route read it, and a caller that rebuilds the record
    // field by field must not drop the trial visitor (it once did, and the
    // visitor lost their own report the moment its text finished).
    const current = await kv.get(`report:${id}`)
    data = {
      ...data,
      ownerId: current?.ownerId || data?.ownerId || null,
      ...((current?.anonId || data?.anonId) && { anonId: current?.anonId || data.anonId }),
    }
    // A report with an owner is kept until its owner deletes it (no TTL); one
    // without (a free-trial visitor's) keeps the 90-day expiry.
    if (data?.ownerId) {
    ...
    const meta = {
      ...
      trial:       !data?.ownerId && !!data?.anonId,
    }
...
function reportSummary(id, data, status = 'complete') {
  return {
    reportId:    id,
    status,
    projectName: data?.projectName || 'Untitled',
 ...
 * A report still "in progress" after a day never finished (its record has
 * expired) and is left out.
 */
export async function listUserReports(uid, now = Date.now()) {
  try {
    const all = (await kv.hgetall(userReportsKey(uid))) || {}
    return Object.values(all)
      .map(v => (typeof v === 'string' ? JSON.parse(v) : v))
      .filter(r => r.status !== 'in-progress' || now - Date.parse(r.generatedAt) < PENDING_TTL * 1000)
...
/** How many reports a user has (for the admin list). 0 if KV is unavailable. */
export async function countUserReports(uid) {
  try {
    return (await kv.hlen(userReportsKey(uid))) || 0
  } catch {
    return 0
  }
}

/** Every report id in a user's list (used when the admin deletes the account). */
export async function userReportIds(uid) {
  return Object.keys((await kv.hgetall(userReportsKey(uid))) || {})
}

/**
 * Give a free-trial report to an account: called for each of a visitor's
 * reports when they sign up or sign in (lib/trial.js). Only a report made by
 * this visitor and not yet owned moves. A finished report loses its expiry; one
 * still generating keeps the 24h pending expiry, and finaliseReport() — which
 * re-reads the owner — makes it permanent. Returns true when it moved.
 */
export async function claimReport(id, anonId, uid) {
  const key = `report:${id}`
  const record = await kv.get(key)
  if (!record || record.ownerId || !anonId || record.anonId !== anonId) return false
  const owned = { ...record, ownerId: uid }
  const complete = !record.status || record.status === 'complete'
  if (complete) await kv.set(key, owned)
  else await kv.set(key, owned, { ex: PENDING_TTL })
  await kv.hset(userReportsKey(uid), { [id]: reportSummary(id, owned, complete ? 'complete' : 'in-progress') })
  return true
}
...
// ─── Weekly usage counts ──────────────────────────────────────────────────────
// One hash, field "<ISO week>:<user|anon>" → reports created that week, counted
// when Phase 1 succeeds (the same moment a free report is used up).
const STATS_KEY = 'stats:reports:weekly'

export async function recordReportStat(kind, date = new Date()) {
  try {
    await kv.hincrby(STATS_KEY, `${isoWeek(date)}:${kind === 'anon' ? 'anon' : 'user'}`, 1)
  } catch (e) {
    console.warn('[KV] recordReportStat skipped:', e.message)
  }
}

/** Reports per ISO week, `{ "2026-W39": { user, anon } }`. {} if KV is unavailable. */
export async function reportStats() {
  try {
    const all = (await kv.hgetall(STATS_KEY)) || {}
    const weeks = {}
    for (const [field, n] of Object.entries(all)) {
      const [week, kind] = field.split(':')
      weeks[week] = weeks[week] || { user: 0, anon: 0 }
      weeks[week][kind] = Number(n) || 0
    }
    return weeks
  } catch (e) {
    console.warn('[KV] reportStats failed:', e.message)
    return {}
  }
}
