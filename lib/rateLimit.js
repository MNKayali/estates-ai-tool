 ...
 * /api/check-access (the retired shared-code login) attempt, which is itself a cheap way for an anonymous
 ...
 * @param {{requests: number, window: string, key?: string}} opts - e.g. {requests: 10, window: '10 m'}
 *   (window syntax: "<number> <ms|s|m|h|d>", per @upstash/ratelimit). `key`
 *   replaces the caller's IP as the bucket — e.g. an email address, so a
 *   password-guessing run spread over many IPs is still bounded per account.
 */
export async function checkRateLimit(name, request, { requests, window, key }) {
  try {
    const limiter = getLimiter(name, requests, window)
    const { success, reset } = await limiter.limit(key || clientIp(request))
    return {
      allowed: success,
