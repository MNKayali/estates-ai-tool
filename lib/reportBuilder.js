...
// The "Cost Risk" badge used to be the E-code (Risk Allowance %) RAG banding —
// a fixed function of one percentage, entirely disconnected from what the risk
// register actually says. A report could show "Cost Risk: Low" on the cover
// while its own register carried a High-rated cost risk. The badge now reflects
// the highest-rated Cost-category entry the AI actually wrote, falling back to
// the percentage banding only while no risk register exists yet (Phase 1
// pending state, before the AI call has run).
...
function infoBoxes(cost, programme) {
  ...
  return tbl(
    [tblRow([
      box('Total Project Cost Range', `${f1k(cost?.total?.low)} – ${f1k(cost?.total?.high)}`, `Excl. VAT  |  ${f1k(cost?.vat)} VAT at 20% (mid-point, ref)`),
      box('Programme', `${programme?.totalWeeks} weeks`,
        [programme?.floatWeeks > 0 ? `Incl. ${programme.floatWeeks} wks float (best case ${programme.totalWeeksBestCase} wks)` : null, targetNote].filter(Boolean).join('  |  '),
        targetColor),
...
function worksTable(lineItems) {
  ...
  function groupRow(item) {
    return tblRow([
      cell(
        para([run(groupHeading(item, GROUP_NAMES), 17, WHITE, true)]),
  ...
  function dataRow(item, idx) {
    ...
    return tblRow([
      ...
      cell(para([
        run(item.description || '', 18, TXT),
        ...(lineBasis(item) ? [new TextRun({ text: sanitize(lineBasis(item)), font: 'Arial', size: 15, color: MID_GR, break: 1 })] : []),
      ]), { w: COLS[1], bg, borders: brd }),
...
function totalCostTable(cost) {
  ...
  for (const b of belowLineRows(cost)) rows.push(dataR(b.label, b.basis, b.low, b.high, ri++))
  const vatPct = cost.vatPct > 0 ? cost.vatPct : 20
  rows.push(dataR('TOTAL PROJECT COST (excl. VAT)', '', tL, tH, ri++, true))
  rows.push(dataR(`VAT @ ${vatPct}% (reference — recoverability to be confirmed)`, `${vatPct}%`, tL * vatPct / 100, tH * vatPct / 100, ri++, false, true))

  return new Table({ width: { size: W, type: WidthType.DXA }, columnWidths: COLS, borders: NO_B, rows })
}

// ─── Percentage build-up table ───────────────────────────────────────────────
// Mirrors BuildUpTable in app/report/ReportRenderer.jsx — both read the same
// rows from lib/reportShared.js's buildUpRows().
function buildUpTable(cost) {
  const rows = buildUpRows(cost)
  if (rows.length === 0) return empty()
  const COLS = [900, 6926, 1200]
  const headerRow = tblRow([
    hdrCell('Code', COLS[0]),
    hdrCell('Workbook rule / trigger', COLS[1]),
    hdrCell('%', COLS[2], { align: AlignmentType.RIGHT }),
  ])
  const body = rows.map(r => {
    const bg = r.subtotal ? ALT_ROW : WHITE
    const col = r.subtotal ? NAVY : TXT
    const b = thinB(BORDER)
    return tblRow([
      cell(para([run(r.subtotal ? '' : r.code, 18, SUBTXT)], { after: 0 }), { w: COLS[0], bg, borders: b }),
      cell(para([run(r.label, 18, col, r.subtotal)], { after: 0 }), { w: COLS[1], bg, borders: b }),
      cell(para([run(r.subtotal ? fmtPlainPct(r.pct) : fmtSignedPct(r.pct), 18, col, r.subtotal)], { align: AlignmentType.RIGHT, after: 0 }), { w: COLS[2], bg, borders: b }),
    ])
  })
  return new Table({ width: { size: W, type: WidthType.DXA }, columnWidths: COLS, borders: NO_B, rows: [headerRow, ...body] })
}
...
function programmeTable(stages, totalWeeks) {
  ...
  stages.forEach((s, i) => {
    ...
    const dates = stageDateRange(s)
    rows.push(tblRow([
      cell(para([run(s.stage || '', 18, NAVY, true)], { after: 0 }), { w: COLS[0], bg: altRow(i), borders: thinB(BORDER) }),
      cell(dates
        ? [para([run(s.activity || '', 18, TXT)], { after: 0 }), para([run(dates, 14, GRAY)], { after: 0 })]
        : para([run(s.activity || '', 18, TXT)], { after: 0 }),
        { w: COLS[1], bg: altRow(i), borders: thinB(BORDER) }),
...
function ganttTable(stages, totalWeeks, surveyWeeks, programme) {
  ...
  return [
    ...
    para([run(`Gantt shows indicative durations. Surveys run concurrently with early design stages.${programme?.startDate && programme?.endDate ? ` Week 1 = ${fmtDate(programme.startDate)}${programme.startDateAssumed ? ' (start assumed)' : ''}; completion around ${fmtDate(programme.endDate)}.` : ''}`, 15, GRAY, false, true)], { after: 60 }),
  ]
}
...
function scopeParas(lineItems) {
  ...
  for (const [grp, items] of Object.entries(groups)) {
    result.push(para([run(heads[grp], 18, NAVY, true)], { before: 80, after: 20 }))
...
function estimateBasisParas(cost, programme, dateStr) {
  ...
  const baseDate = baseDateLabel(cost)
  const items = [
    ...
    // Conditional, not boilerplate: a short programme can legitimately fall
    // below the Tab 3 inflation trigger bands for both tender delay and
    // construction duration, in which case no allowance is priced. Asserting
    // an allowance that isn't in the cost table (as this sentence used to,
    // unconditionally) states something false about the actual figures.
    inflationPct > 0
...
// Scope reconciliation audit — every element priced beyond what was ticked
// (autoIncludes), and every ticked element that was combined, superseded, or
// hidden rather than priced directly (adjustments), each with its declared
// reason. This is what lets a user check "priced lines = ticked lines" for
// themselves rather than trusting the total silently — see costCalculator.js's
// reconciliation invariant, which guarantees every one of these has a reason.
function scopeReconciliationParas(cost) {
  const items = [
...
export async function buildReport({ answers, cost, programme, aiProse, budget }) {
  ...
  // One resolver for all three outputs — see resolveSectionFlags in lib/reportShared.js.
  const { showCost, showROI, showProc, showCon } = resolveSectionFlags(answers, roi)
  ...
  const content = [
    ...
    para([
      ...
      run(programmeHeadline(programme), 20, TXT),
      run('     |     ', 20, SUBTXT),
    ...
    ...(programme?.stages?.length > 0 ? [
      ...
      ...(programme.stages.some(s => s.parallel)
        ...
        : []),
      empty(),
      ...(ganttTable(programme.stages, programme.totalWeeks, programme.surveyWeeks, programme) || []),
    ] : []),
    ...
    ...(programme?.fastTrackOptions?.length > 0 ? [
      subHdr('Programme Acceleration Options'),
      para([run('The headline programme does not assume any of these. Each is a lever from the Programme workbook whose trigger applies to this project, with the trade-off the client would accept to use it.', 18, SUBTXT, false, true)], { after: 60 }),
      ...fastTrackLines(programme).map(l => bul(l)),
    ] : []),
    ...
    para([
      run(`${areaLabel(cost.projectType || answers?.q1_2_projectType)}: `, 20, NAVY, true), run(`${cost.gifa} m²`, 20, TXT),
      run('   |   ', 20, SUBTXT),
    ...
    ...(cost.trace ? [
      subHdr('How each percentage was derived'),
      para([run('Every percentage above is the sum of the NRM1 workbook rules listed here, each triggered by a questionnaire answer. Nothing is applied that is not shown.', 18, SUBTXT, false, true)], { after: 60 }),
      buildUpTable(cost),
      empty(),
    ] : []),
    ...
    para([
      ...
      run(deriveCostRiskLevel(cost, aiProse), 20, TXT),
    ], { after: 80 }),
    ...
    ...((cost.autoIncludes?.length || cost.adjustments?.length) ? [
      subHdr('Scope Selected vs Priced'),
      para([run('Every line item priced above is either something you ticked, or one of the auto-included items below with a stated reason. Nothing else is added silently.', 20, SUBTXT, false, true)], { after: 60 }),
      ...scopeReconciliationParas(cost),
    ] : []),
    ] : []),   // end showCost
    ...
    ...(showProc ? [
      ...
      para([
        ...
        run(aiProse?.procurementRoute || programme?.procurementRoute || '', 20, TXT),
        run('   |   ', 20, SUBTXT),
        run('Contract: ', 20, NAVY, true),
        run(aiProse?.procurementContractForm || programme?.contractForm || '', 20, TXT),
      ], { after: 60 }),
      para([
        run('Design responsibility: ', 20, NAVY, true),
        run(aiProse?.procurementDesignResp || programme?.designResponsibility || '', 20, TXT),
        run('   |   ', 20, SUBTXT),
        run('Tender type: ', 20, NAVY, true),
        run(aiProse?.procurementTenderType || programme?.tenderType || '', 20, TXT),
        ...(programme?.procurementSource ? [
          run('   |   ', 20, SUBTXT),
          run(`Selected by: ${programme.procurementSource}`, 16, SUBTXT),
        ] : []),
      ], { after: 100 }),
