...
function run(text, sz = 20, color = TXT, bold = false, italic = false) {
  return new TextRun({ text, font: 'Arial', size: sz, color, bold, italic })
}
...
const bul = (text, ref = 'bullets') =>
  para([run(text, 20, TXT)], { after: 40, num: { reference: ref, level: 0 } })
...
function infoBoxes(cost, programme) {
  ...
  return tbl(
    [tblRow([
      box('Total Project Cost Range', `${f1k(cost?.total?.low)} – ${f1k(cost?.total?.high)}`, 'Excl. VAT'),
...
function worksTable(lineItems) {
  ...
  // Header row
  const hdrs = ['Code', 'Element', 'Low £/m²', 'High £/m²', 'Total Low £', 'Total High £']
  ...
  function dataRow(item, idx) {
    ...
    return tblRow([
      ...
      cell(para([run(f1k(item.lineLow || 0), 18, TXT)], { align: AlignmentType.RIGHT, after: 0 }), { w: COLS[4], bg, borders: brd }),
      cell(para([run(f1k(item.lineHigh || 0), 18, TXT)], { align: AlignmentType.RIGHT, after: 0 }), { w: COLS[5], bg, borders: brd }),
    ])
  }
...
function riskTable(risks) {
  ...
  const COLS = [500, 1200, 2726, 400, 400, 700, 3100]
  const hdrs = ['Ref', 'Category', 'Description', 'L', 'I', 'Rating', 'Mitigation']
  const headerRow = tblRow(hdrs.map((h, i) =>
  ...
  function ragCell(val, cw) {
    const bg  = RAG_BG[val]  || ALT_ROW
    const col = RAG_COL[val] || TXT
    return cell(
      para([run(val || '', 17, col, true)], { align: AlignmentType.CENTER, after: 0 }),
  ...
  risks.forEach((r, i) => {
    rows.push(tblRow([
      cell(para([run(r.ref || `R${String(i+1).padStart(2,'0')}`, 17, NAVY, true)], { after: 0 }), { w: COLS[0], bg: altRow(i), borders: thinB(BORDER) }),
      cell(para([run(r.category || '', 17, TXT)], { after: 0 }), { w: COLS[1], bg: altRow(i), borders: thinB(BORDER) }),
      cell(para([run(r.description || '', 17, TXT)], { after: 0 }), { w: COLS[2], bg: altRow(i), borders: thinB(BORDER) }),
      ragCell(r.likelihood, COLS[3]),
      ragCell(r.impact, COLS[4]),
      ragCell(r.rating, COLS[5]),
      cell(para([run(r.mitigation || '', 17, TXT)], { after: 0 }), { w: COLS[6], bg: altRow(i), borders: thinB(BORDER) }),
    ]))
  })
...
export async function buildReport({ answers, cost, programme, aiProse }) {
  ...
  const content = [
    ...
    ...(programme?.stages?.length > 0 ? [programmeTable(programme.stages, programme.totalWeeks), empty()] : []),
    ...
    ...sectionHdr('5', 'Order of Cost Estimate (NRM1)'),
    para([
    ...
    // ── Section 6 — ROI (conditional) ──
    ...(roi ? [
      ...sectionHdr('6', 'ROI & Financial Case'),
      para([
        run('Benefit type: ', 20, NAVY, true),
        run(answers.q5_1_financialBenefit || '—', 20, TXT),
        run('   |   ', 20, SUBTXT),
    ...
    ...sectionHdr('7', 'Procurement Recommendation'),
    para([
    ...
    // ── Section 8 — Constraints (conditional) ──
    ...(aiProse?.constraints?.length > 0 ? [
      ...sectionHdr('8', 'Constraints Summary'),
      constraintsTable(aiProse.constraints),
      empty(),
    ] : []),

    // ── Section 9 — Next Steps ──
    ...sectionHdr('9', 'Recommendations & Next Steps'),
