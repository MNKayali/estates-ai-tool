...
function buildProgRows(stages) {
  return (stages || []).map(s => ({
    stage:       s.stage,
    activity:    s.activity,
    durationWks: String(s.weeks ?? s.durationWks ?? 0),  // support both field names
    weeks:       String(s.weeks ?? s.durationWks ?? 0),
  }))
}
...
export async function buildReport({ answers, cost, programme, aiProse }) {
  ...
  // Build prog assumptions (use computed assumptions — no workbook placeholders)
  const progAssumptionsText = (programme.assumptions || programme.standardAssumptions || []).map(a => `- ${a}`).join('\n')
