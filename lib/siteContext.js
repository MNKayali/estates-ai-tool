 ...
 *
 * A report whose user left the Q3.8 tick UNticked has the opposite quirk:
 * re-run through /api/compare, it can now derive true off the Section 1
 * answers it already had, and the compare panel can show a Gateway 2 stage
 * the originally printed report doesn't. Accepted, not fixed — spec §9: a
 * 7+ storey residential building IS higher-risk under the statute regardless
 * of what the old tick captured, and /api/compare already diverges from an
 * older report whenever a workbook rate or duration changes.
 */
