...
import { isQuestionShown } from './questionSets.js'
import {
...
export function coverSubtitle(answers, cost) {
  ...
  return [place, buildingUseLabel(answers), gifa].filter(Boolean).join(' · ')
}

/** Q1.3 as the reader should see it: the client's own description when they chose "Other". */
export function buildingUseLabel(answers) {
  const use = String(answers?.q1_3_buildingUse || '').trim()
  const other = String(answers?.q1_3_buildingUseOther || '').trim()
  return use === 'Other' && other ? other : use
}

/**
 * Is the specification level something the client chose? Q2.4 is not asked
 * for project types with a single rate column (External works only) or where
 * it does not apply (Demolition only); the engine still prices at a default
 * level there, but printing "Specification: Standard" read as an answer the
 * client never gave (test reports EW-1/2/3, DM-1/2/3).
 */
//
// Decided by whether the form ASKS Q2.4 for the project type, not by whether
// an answer is stored: for External works only the questionnaire stores a
// default "Standard" without showing the question (batch-2 test run, EW-4/5/6).
export const specShown = (answers, cost) =>
  !!cost?.specLevel && !!String(answers?.q2_4_specLevel || '').trim() &&
  isQuestionShown('q2_4_specLevel', answers?.q1_2_projectType || cost?.projectType)

/** The cover's project facts (left column) — both renderers print these rows. */
export function coverFactRows(answers, cost) {
  return [
    ['Project type', cost?.projectType || answers?.q1_2_projectType || ''],
    ...(cost?.interventionLevel ? [['Intervention', cost.interventionLevel]] : []),
    ...(specShown(answers, cost) ? [['Specification', cost.specLevel]] : []),
  ]
}

/** Q2.5 standards, as a list ('None' dropped). Stored as a comma-joined string or an array. */
export function standardsList(answers) {
  const raw = answers?.q2_5_standards
  const list = Array.isArray(raw) ? raw : String(raw || '').split(',')
  const other = String(answers?.q2_5_standardsOther || '').trim()
  return list.map(s => String(s).trim()).filter(s => s && !/^none$/i.test(s))
    .map(s => (s === 'Other' && other ? other : s))
}
...
/** "11 Jan 27" — the dense programme table's date format, shared by both renderers. */
export const fmtShortDate = iso => fmtDate(iso).replace(/ \d{2}(\d{2})$/, ' $1')
...
const NO_RETURN = /no direct financial return/i

/**
 * The Financial Case, or null when there isn't one. Shared by both renderers
 * and the AI prompt, so the section, its figures and its narrative agree.
 *
 * It exists when Q5.1 names a financial benefit, or Q5.2 gives an annual
 * figure. It used to need the Q5.2 figure, so a client who ticked "Rental or
 * commercial income" without a number lost the whole section (test reports
 * EW-3, FO-3, OM-3, RF-3). "No direct financial return" alone is a strategic
 * or compliance project: there is no case to make, so no section.
 *
 * The mid-point is the estimate's own mid (cost.total.mid). Since the range
 * widths became asymmetric (-20 % / +25 % at grade C) the average of the low
 * and high ends is not the mid, and the payback disagreed with the mid-point
 * printed elsewhere (NB-3, OM-1, RF-2).
 */
export function calcRoi(answers, cost) {
  const annual = Number(answers?.q5_2_annualBenefit) || 0
  const raw = answers?.q5_1_financialBenefit
  const benefits = (Array.isArray(raw) ? raw : raw ? [raw] : []).map(String).filter(b => b.trim() && !NO_RETURN.test(b))
  const { low = 0, high = 0 } = cost?.total || {}
  if (!(low > 0 && high > 0) || (!annual && benefits.length === 0)) return null
  const base = cost.total.mid > 0 ? cost.total.mid : (low + high) / 2
  const mid = Math.round(base / 1000) * 1000
  return { annual, mid, benefits, paybackYears: annual ? Math.round((mid / annual) * 10) / 10 : null }
}

// ─── Risk register ───────────────────────────────────────────────────────────
export const RAG_CLASS = Object.freeze({ High: 'high', Medium: 'med', Low: 'low' })

// A seed's own Ref beats its seedRef tag: the model has tagged one seed with
// another's Ref, which made the two collide here and dropped one (test run).
const riskKey = r => {
...
export function prepareRisks(register, max = LIMITS.maxRisks) {
  ...
  // Within a rating, deterministic seeds (from the client's own answers) come
  // before the model's additions, so the cap never drops a risk the client told
  // us about in favour of one the model inferred.
  const seeded = r => riskKey(r).startsWith('seed:') ? 0 : 1
  const ordered = kept
    .map((r, i) => ({ r, i }))
    .sort((a, b) => (RISK_RANK[b.r.rating] || 0) - (RISK_RANK[a.r.rating] || 0) || seeded(a.r) - seeded(b.r) || a.i - b.i)
    .map(x => x.r)
    .slice(0, max)
...
/**
 * 'compact' when the register would not fit its page at the normal size —
 * reports written before the length caps. Both renderers use it, so the risk
 * register stays on one page everywhere.
 */
export function riskTableMode(risks) {
  const h = estimateHeight([{ table: { widths: TABLES.risk, rows: risks.map(r => [r.ref, r.category, r.description, r.rating, r.mitigation]) } }])
  return h > PAGE.bodyPx - 150 ? 'compact' : 'normal'
}
...
export function programmeNarrativeLines(programme) {
  ...
  // Batch-2 EW-6: a prior-approval project with its design at Stage 4 read
  // "No planning application is assumed" — consent is needed, just taken as held.
  lines.push(programme.planningWeeks > 0
    ? (programme.planningAlongside === 'Stage 4'
      ? `A ${programme.planningWeeks}-week planning determination runs alongside technical design and holds tender until consent is granted.`
      : `A ${programme.planningWeeks}-week planning determination gates technical design.`)
    : programme.planningStatus === 'held'
      ? 'Planning consent is taken as already obtained, as the design is complete to Stage 4; confirm it before tender.'
...
/**
 * Rounds `values` to `step` so that they add up exactly to `total` rounded to
 * the same step (largest-remainder method): each line is rounded, then the
 * lines closest to rounding the other way absorb the difference. Rounding
 * every line on its own let 14–22 lines drift up to a few thousand pounds
 * from their printed total (test reports DM-1, FO-6, RF-5, RF-6).
 */
export function footedRound(values, total, step) {
  const rounded = values.map(v => Math.round((v || 0) / step) * step)
  let diff = Math.round((Math.round((total || 0) / step) * step - rounded.reduce((a, b) => a + b, 0)) / step)
  const order = values.map((v, i) => ({ i, rem: (v || 0) - rounded[i] }))
    .sort((a, b) => (diff > 0 ? b.rem - a.rem : a.rem - b.rem))
  for (let k = 0; diff !== 0 && k < order.length * 2; k++) {
    const { i } = order[k % order.length]
    if (diff < 0 && rounded[i] < step) continue
    rounded[i] += Math.sign(diff) * step
    diff -= Math.sign(diff)
  }
  return rounded
}

/** Works table rows; each line carries `low`/`high` rounded so the column foots to the works total. */
export function worksRows(cost) {
  const items = cost?.lineItems || []
  const step = roundingStep(cost?.total?.high)
  const lows = footedRound(items.map(li => li.lineLow), cost?.works?.low ?? items.reduce((a, li) => a + (li.lineLow || 0), 0), step)
  const highs = footedRound(items.map(li => li.lineHigh), cost?.works?.high ?? items.reduce((a, li) => a + (li.lineHigh || 0), 0), step)
  const rows = []
  let g
  items.forEach((li, i) => {
    if (li.group !== g) { rows.push({ type: 'group', group: li.group, label: groupTitle(li) }); g = li.group }
    rows.push({ type: 'line', item: li, low: lows[i], high: highs[i] })
  })
  return rows
}
...
export function worksGroupRows(cost) {
  const m = new Map()
  for (const row of worksRows(cost).filter(r => r.type === 'line')) {
    const li = row.item
...
// Workbook units such as "nr trees" or "nr chargers" already name the thing
// counted, so they print as "8 trees" / "1 charger", not "1 nr trees" (test
// reports EW-1, EW-2).
export const lineQty = li => {
  if (li.unit === 'band') return 'lump sum'
  const named = String(li.unit || '').match(/^nr\s+(.+)$/i)
  if (named) return `${fmtQty(li.qty)} ${Number(li.qty) === 1 ? named[1].replace(/s$/, '') : named[1]}`
  return `${fmtQty(li.qty)} ${li.unit || ''}`.trim()
}
...
// "a standard-specification refurbishment" — or just "a refurbishment" where
// Q2.4 was not asked. `answers` is optional: without it the level is named.
const specPhrase = (cost, answers) => (answers && !specShown(answers, cost) ? '' : `${String(cost.specLevel || 'standard').toLowerCase()}-specification `)
// External works only has no floor area (GIFA 0): quantities come from the site area.
const areaPhrase = cost => (Number(cost.gifa) > 0 ? `the ${Number(cost.gifa).toLocaleString('en-GB')} m² floor area` : 'the site area')

export function costIntroText(cost, answers) {
  if (!cost) return ''
  ...
  return [
    `Each ticked scope item is priced at the NRM1 benchmark rate for a ${specPhrase(cost, answers)}${String(cost.projectType || '').toLowerCase()}, adjusted by the ${cost.bcisRegion} location factor (${cost.bcisFactor}).`,
    user ? `${est} quantities are estimated from ${areaPhrase(cost)} and ${user} are the client's own figures.` : `Quantities are estimated from ${areaPhrase(cost)}.`,
...
export function projectCostRows(cost) {
  ...
  // The cost engine rounds each percentage line to the nearest £1,000 and
  // builds every subtotal from those rounded amounts, so the table foots only
  // if the rows are rounded the same way. Unrounded rows printed at £100 steps
  // on a small project left DM-2's total ~4% off its own rows.
  const r1k = n => Math.round(n / 1000) * 1000
  const of = (base, pc) => ({ low: r1k((base.low || 0) * (pc || 0) / 100), high: r1k((base.high || 0) * (pc || 0) / 100) })
  const rows = [
...
export function percentageLines(cost) {
  ...
  for (const [key, name] of PCT_NAMES) {
    ...
    if (entries.length === 1 && /^always\b/i.test(entries[0].label)) text = 'fixed for every report'
      ...
      // A cap is traced as a negative entry so the list foots; printed as
      // "…, capped at 10%" rather than "-1% for capped at 10%" (test report EW-2).
      const cap = entries.find(e => /^capped at/i.test(e.label))
      const parts = entries.filter(e => e !== cap)
        .map(e => (/^(base|baseline)\b/i.test(e.label) ? `${pctTxt(e.pct)} base` : `${pctTxt(e.pct)} for ${plainRule(e.label)}`))
      text = parts.length > 1 ? `${parts[0]}, plus ${parts.slice(1).join(', ')}` : (parts[0] || '')
      if (cap) text += `, ${cap.label.charAt(0).toLowerCase()}${cap.label.slice(1)}`
    }
...
export function costAssumptionLines(cost, answers) {
  if (!cost) return []
  const lines = (cost.lineItems || []).filter(l => l.code !== 'PS')
  const userLines = lines.filter(l => l.qtySource === 'user')
  const spec = !answers || specShown(answers, cost) ? `a ${String(cost.specLevel || 'standard').toLowerCase()} specification` : ''
  const level = cost.interventionLevel ? String(cost.interventionLevel).toLowerCase() : ''
  const out = [
    ...
    lines.length > userLines.length ? `Estimated quantities are derived from ${areaPhrase(cost)} and confirmed at Stage 2.` : null,
    ...
    spec || level ? `Rates are for ${[spec, level].filter(Boolean).join(' and ')}.` : null,
...
// ─── Scope ───────────────────────────────────────────────────────────────────
export function scopeStatement(cost, answers) {
  if (!cost) return ''
  // Distinct scope items, not priced lines: one item can price several options
  // (standard + accessible toilets), and the AI counts items (batch-2 NB-6: 34 vs 37).
  const n = new Set((cost.lineItems || []).filter(l => l.code !== 'PS').map(l => l.code)).size
  const parts = [
    Number(cost.gifa) > 0 ? `of ${Number(cost.gifa).toLocaleString('en-GB')} m² GIFA` : '',
    !answers || specShown(answers, cost) ? `${String(cost.specLevel || 'standard').toLowerCase()} specification` : '',
    `${n} priced scope item${n === 1 ? '' : 's'}`,
    // Q2.5 used to vanish from the report unless a workbook rule priced it
    // (test reports FO-3, RF-1/2/3). Stated with the scope it governs.
    answers && standardsList(answers).length ? `standards: ${standardsList(answers).join(', ')}` : '',
  ].filter(Boolean)
  const head = `${cost.projectType || 'Project'}${cost.interventionLevel ? ` (${String(cost.interventionLevel).toLowerCase()})` : ''}`
  return parts[0].startsWith('of ') ? `${head} ${parts[0]}, ${parts.slice(1).join(', ')}.` : `${head}: ${parts.join(', ')}.`
}
