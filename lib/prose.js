...
import { hasSiteContext } from '@/lib/siteContext'
...
export function scrubAnswers(answers = {}) {
  ...
  return {
    ...
    vatPosition:       answers.q4_3a_vatPosition,
...
// SITE_CONTEXT key in lib/costCalculator.js that hasSiteContext() tests.
...
function buildDeterministicSeeds(answers, cost, senseCheck) {
  ...
  for (const s of CONTEXT_RISK_SEEDS) {
    if (hasSiteContext(answers, s.contextKey)) seeds.push(s)
  }
...
Target date: ${answers.q4_1_targetDate || 'None specified'} | Budget (incl. fees & VAT): ${answers.q4_3_budget ? f(answers.q4_3_budget) : 'Not stated'} | VAT position: ${answers.q4_3a_vatPosition || 'Not recoverable (assumed)'}${String(answers.q4_3a_vatPosition || '').startsWith('Partially') && answers.q4_3a_vatRecoverablePct ? ` (${answers.q4_3a_vatRecoverablePct}% recoverable)` : ''}
