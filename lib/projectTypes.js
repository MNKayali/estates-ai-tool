...
export const VISIBLE_GROUPS = {
  ...
  // Retired values. NOT for KV report resolution — a finished report never
  // re-enters the picker or suggest-scope, so this table is irrelevant to it.
  // Kept because they are harmless and the suggest-scope route's 400-guard
  // (`if (!VISIBLE_GROUPS[projectType]) return 400`) reads this same map, so
  // an old draft or stored answer set carrying a retired value still passes
  // that check instead of erroring.
  'External Works':      [0, 8],
  'Demolition':          [0],
