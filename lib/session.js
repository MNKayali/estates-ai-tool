 ...
 *
 * The free-trial cookie (projento_trial, below) uses the same signed format
 * with `typ: 'trial'`; each verifier refuses the other's token.
 *
...
export async function createSessionToken({ uid, ver }, maxAgeSeconds = SESSION_MAX_AGE, now = Date.now()) {
  if (!uid) return null
  return signPayload({ uid, ver: ver ?? 0, exp: Math.floor(now / 1000) + maxAgeSeconds })
}

/**
 * Verify a token's signature and expiry. Returns `{ uid, ver, exp }` or null.
 * Does not check the user still exists or is active — see lib/auth.js. A trial
 * token (`typ: 'trial'`) is never accepted as a session.
 */
export async function verifySessionToken(token, now = Date.now()) {
  const payload = await verifyPayload(token, now)
  if (!payload?.uid || payload.typ) return null
  return payload
}

// The signed-payload format both cookies share: `<base64url(JSON)>.<hex HMAC>`.
...
async function verifyPayload(token, now) {
  const s = secret()
...
// ─── Free-trial visitor cookie ────────────────────────────────────────────────
// An anonymous visitor is identified by a random id in a signed cookie; the
// number of reports they have generated is kept in KV against that id
// (lib/trial.js), never in the browser. Same signing and the same fail-closed
// rule as the session cookie.

export const TRIAL_COOKIE = 'projento_trial'
export const TRIAL_MAX_AGE = 60 * 60 * 24 * 365 // 1 year

export async function createTrialToken(tid, maxAgeSeconds = TRIAL_MAX_AGE, now = Date.now()) {
  if (!/^[0-9a-f]{16}$/.test(String(tid || ''))) return null
  return signPayload({ typ: 'trial', tid, exp: Math.floor(now / 1000) + maxAgeSeconds })
}

/** The visitor id in a valid trial token, or null. */
export async function verifyTrialToken(token, now = Date.now()) {
  const payload = await verifyPayload(token, now)
  if (payload?.typ !== 'trial' || !/^[0-9a-f]{16}$/.test(String(payload.tid || ''))) return null
  return payload.tid
}
