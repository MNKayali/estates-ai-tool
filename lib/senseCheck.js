...
// VAT position (Q4.3a). The share of VAT the client recovers — 0 for a
// non-recoverable client (the default, and the only case before September
// 2026), 1 for fully recoverable, or the stated percentage for partial
// recovery (universities and NHS trusts commonly recover a fraction).
export function vatRecoverableShare(answers) {
  const pos = String(answers?.q4_3a_vatPosition || '').toLowerCase()
  if (pos.startsWith('fully')) return 1
  if (pos.startsWith('partially')) {
    const pct = Number(answers?.q4_3a_vatRecoverablePct)
    return Number.isFinite(pct) && pct > 0 ? Math.min(pct, 100) / 100 : 0
  }
  return 0
}
...
export function budgetVerdict(answers, cost) {
  ...
  // VAT rate comes from the NRM1 workbook's G row (via cost.vatPct); the
  // recoverable share from the questionnaire. Gross = net × (1 + rate × the
  // unrecovered share), so a fully recoverable client's budget is compared
  // against the net estimate and a non-recoverable one against net + VAT.
  const vatPct = Number.isFinite(Number(cost?.vatPct)) && Number(cost?.vatPct) > 0 ? Number(cost.vatPct) : 20
  const share = vatRecoverableShare(answers)
  const grossFactor = 1 + (vatPct / 100) * (1 - share)
  const vatNote = share >= 1
    ? 'incl. fees; VAT fully recoverable so compared against the net estimate'
    : share > 0
      ? `incl. fees and the unrecovered ${Math.round((1 - share) * 100)}% of VAT`
      : 'incl. fees and VAT'
