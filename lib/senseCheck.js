...
export async function runSenseCheck(cost, programme, answers = {}) {
  ...
  if (bench) {
    ...
    // gap in OUR workbook (Sheet 8 has no row for this combination), not
    ...
    warnings.push({
      ...
      message: benchUnavailable
        ? `Benchmark sheet could not be read (${benchUnavailable}) — the cost-per-m² check did not run. ` +
          `The estimate has not been validated against any published band.`
        : `Sheet 8 has no benchmark band for "${cost.projectType}" / "${answers.q1_3_buildingUse || '(none)'}" — ` +
