...
import {
  coverTitle, coverTitleSize, coverSubtitle, coverCostRange, confidenceWord, deriveCostRiskLevel, cleanReportText, vatPct,
...
import { t, p, cell, band, table, pill, exactRow, rule, newTable, CONTENT_DXA, PAGE_DXA } from './primitives.js'
...
export function coverPage(data, ctx) {
  ...
  const top = exactRow(TOP, [cell([
    p([t(` ${BRAND.mark} `, { semi: true, color: COLOURS.paper, size: COVER_TYPE.subtitle, fill: COLOURS.amberOnDark }), t(`  ${BRAND.name}`, { semi: true, color: COLOURS.paper, size: COVER_TYPE.brand })], { before: 640, after: 60 }),
    p(t(BRAND.tagline, { color: COLOURS.onNavyMute, size: COVER_TYPE.tagline }), { after: 1500 }),
  ...
  const left = [['Project type', cost?.projectType || answers?.q1_2_projectType || ''], ...(cost?.interventionLevel ? [['Intervention', cost.interventionLevel]] : []), ['Specification', cost?.specLevel || '']]
  ...
  const foot = exactRow(FOOT, [cell(new Paragraph({
    ...
    children: [
      ...
      t(`\t${BRAND.slogan}`, { size: COVER_TYPE.note, color: COLOURS.onNavy, italic: true }),
    ],
...
export function scopePage(data) {
  ...
  const out = [band(2, 'Scope of Works'), gap(160), p(t(scopeStatement(cost))), h3('Included works'), scopeGrid(scopeGroups(cost))]
...
const worksLineRows = (rows, m) => rows.map(r => (r.type === 'group'
  ...
  : { kind: 'row', cells: [
      ...
      lineQty(r.item), lineBasisWord(r.item), m(r.item.lineLow, true), m(r.item.lineHigh, true),
    ] }))
...
export function costWorksPage(data, ctx) {
  ...
  const out = [
    ...
    p(t(aiProse?.costNarrative ? cleanReportText(aiProse.costNarrative) : costIntroText(cost), { color: COLOURS.grey })),
  ]
  if (mode === 'lines') {
    out.push(table({
...
export function costSummaryPage(data, ctx) {
  ...
  out.push(grid([0.5, 0.5], [
    w => cell([h3('Cost assumptions'), ...bullets(costAssumptionLines(cost))], { width: w, margins: { top: 0, bottom: 0, left: 0, right: 240 } }),
...
export function lateSection(key, no, data, ctx) {
  ...
  if (key === 'roi') {
    ...
    return [
      ...
      grid([0.25, 0.25, 0.25, 0.25], [fig('Project cost (mid)', m(roi?.mid)), fig('Annual benefit', m(roi?.annual)), fig('Simple payback', `${roi?.paybackYears} years`), fig('Benefit type', benefit, true)]),
      gap(160),
...
export function lastPage(data, ctx, no) {
  ...
  const contact = grid([0.32, 0.68], [
    w => cell([
      p([t(` ${BRAND.mark} `, { semi: true, color: COLOURS.paper, fill: COLOURS.amberOnDark, size: TYPE.table }), t(`  ${BRAND.name}`, { semi: true, color: COLOURS.navy, size: TYPE.subHeading })], { after: 60 }),
      p(t(BRAND.strapline, { size: TYPE.small, color: COLOURS.greyMute }), { after: 0 }),
    ...
    w => cell([
      ...
      p([t('For questions about this report, or to take the project on to a full cost plan and Stage 2 brief, contact our team and quote reference ', { size: TYPE.table }), t(ctx.reference, { mono: true, size: TYPE.table })], { after: 120 }),
      table({ widths: [0.22, 0.78], width: w - 500, rows: [kv('Email', BRAND.email), kv('Telephone', BRAND.phone), kv('Web', BRAND.web)] }),
