...
// ── M&E scope detection ───────────────────────────────────────────────────────
// Used by costCalculator (≥4 M&E risk trigger) and programmeCalculator
// (DS3a/DS4a duration extension). Single source of truth so both callers agree.

export const ME_KEYWORDS = [
  'heating', 'ventilation', 'air conditioning', 'air handling', 'cooling',
  'plumbing', 'sprinkler', 'fire suppression', 'gas',
...
/**
 * Returns the count of M&E scope items.
 * NRM1 Group 5 codes (5.x) are authoritative; keyword matching is a fallback
 * for any text-labelled specialist items.
 */
export function countMeItems(scopeItems) {
  return (scopeItems || []).filter(
    item => /^5\./.test(String(item)) ||
      ME_KEYWORDS.some(kw => String(item).toLowerCase().includes(kw))
  ).length
}
