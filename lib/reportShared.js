...
export function unverifiedRatesSentence(cost) {
  ...
  return `${UNVERIFIED_MARK} ${flagged.length} of ${lines.length} priced lines use a rate still to be verified in the rate library (${[...new Set(flagged.map(li => li.item || li.description))].join(', ')}). Treat these as unverified until they are checked against real project examples.`
}
