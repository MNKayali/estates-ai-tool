...
// Test reports NB-3 and OM-3: "Developed design (Stage 3)" + "Full planning"
// printed a programme with no planning in it, while the cost build-up applied
// the full-planning rate. Under the RIBA Plan of Work the application goes in
// at the END of Stage 3, so the determination is still to come.
describe('programmeCalculator — planning after a completed Stage 3', () => {
  const P = { ...BASE, q3_4_planningConsents: 'Full planning' }
  it('keeps the planning determination, alongside Stage 4', async () => {
    const p = await calculateProgramme({ ...P, q4_5_designStage: 'Developed design (Stage 3)' }, 800_000)
    const planning = p.stages.find(s => s.stage === 'Planning')
    expect(planning).toBeTruthy()
    expect(planning.activity).toContain('Stage 4')
    const s4 = p.stages.find(s => s.stage === 'Stage 4')
    expect(planning.startWeek).toBe(s4.startWeek)
    expect(p.planningWeeks).toBeGreaterThan(0)
  })
  it('holds tender for any determination that outlasts Stage 4, and counts it', async () => {
    const p = await calculateProgramme({ ...P, q4_5_designStage: 'Developed design (Stage 3)' }, 800_000)
    const planning = p.stages.find(s => s.stage === 'Planning')
    const tender = p.stages.find(s => s.stage === 'Tender / Procurement')
    expect(tender.startWeek).toBeGreaterThanOrEqual(planning.endWeek)
  })
  it('still assumes consent is held once Stage 4 is complete', async () => {
    const p = await calculateProgramme({ ...P, q4_5_designStage: 'Technical complete (Stage 4)' }, 800_000)
    expect(p.stages.find(s => s.stage === 'Planning')).toBeUndefined()
  })
  it('is unchanged for a Stage 0–1 project (planning alongside Stage 3)', async () => {
    const p = await calculateProgramme(P, 800_000)
    const planning = p.stages.find(s => s.stage === 'Planning')
    expect(planning.startWeek).toBe(p.stages.find(s => s.stage === 'Stage 3').startWeek)
  })
})

// Batch-2 EW-6: prior approval with the design at Stage 4 read "No planning application is assumed".
describe('programmeCalculator — planning status for the narrative', () => {
  const P = { ...BASE, q3_4_planningConsents: 'Prior approval' }
  it('reports consent as held once Stage 4 is complete, a determination otherwise, none when not needed', async () => {
    expect((await calculateProgramme({ ...P, q4_5_designStage: 'Technical complete (Stage 4)' }, 500_000)).planningStatus).toBe('held')
    const s3 = await calculateProgramme({ ...P, q4_5_designStage: 'Developed design (Stage 3)' }, 500_000)
    expect([s3.planningStatus, s3.planningAlongside]).toEqual(['determination', 'Stage 4'])
    expect((await calculateProgramme(BASE, 500_000)).planningStatus).toBe('none')
  })
})
