...
// The question that set these keys has been removed. New reports carry neither
// key and include everything; the two legacy branches stay alive only for
// reports sitting in KV, which lasts 90 days.
describe('resolveSectionFlags — every section included, legacy keys still honoured', () => {
  it('a new report, with no section keys at all, includes everything', () => {
    expect(resolveSectionFlags({}, { paybackYears: 3 })).toEqual({ showCost: true, showROI: true, showProc: true, showCon: true })
  })
  it('honours a legacy exclude-list from a report already in KV', () => {
  ...
  it('hides ROI when there is no ROI data, whatever the keys say', () => {
