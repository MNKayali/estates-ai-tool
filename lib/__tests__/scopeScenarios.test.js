...
import {
  ...
  defaultOptionKeys, indexCatalogue, lacksRate,
} from '../scopeEngine.js'
...
describe('Scenario 5 — car-park EV scheme (EW · 2,500 m² external area)', () => {
  ...
  it('offers Solar PV, supply upgrade and EV charging with no floor area, and raises the DNO entry', async () => {
    ...
    // Workbook v5.3: "Use typical scope" ticks the app's previous External works preset.
    expect(typicalItemIds(cat, ctx).map(name).sort()).toEqual([
      'Fences, gates and walls', 'Roads, paths and paving', 'Site clearance and preparation', 'Soft landscaping', 'Surface water drainage',
    ])
...
describe("'Shown on' decides visibility, not the rate columns", () => {
  it('an item with no rate for the project type still appears, is never pre-ticked, and prices as unpriced', async () => {
    const pv = cat.items.find(i => i.name === 'Solar PV')
    const bare = { ...pv, options: pv.options.map(o => ({ ...o, priceableFor: { ...o.priceableFor, EW: false } })) }
    const copy = { ...cat, items: cat.items.map(i => (i === pv ? bare : i)) }
    const ctx = buildContext(copy, { ...BASE, q1_2_projectType: 'External works only', q1_3_buildingUse: 'Commercial offices', q1_5_size: '2500' })
    expect(isOffered(bare, ctx)).toBe(true)
    expect(lacksRate(bare, ctx)).toBe(true)
    expect(typicalItemIds(copy, ctx)).not.toContain(bare.id)
  })
})
