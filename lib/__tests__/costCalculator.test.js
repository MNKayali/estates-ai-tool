...
describe('cost — Other or mixed rate fallback', () => {
  ...
  it('5.1b stays excluded on Other or mixed — no rate in any family, so the fallback cannot rescue it', async () => {
    const cost = await calculateCost({ ...MIXED, q2_2_scopeItems: ['1.1', '1.3', '3.1', '5.1b'] }, 0)
    expect(cost.lineItems.map(l => l.code)).not.toContain('5.1b')
    const excluded = cost.excludedNoQuantity.find(e => e.code === '5.1b')
    expect(excluded).toBeDefined()
    expect(excluded.reason).toMatch(/no applicable rate/i)
    expect(cost.rateFallbacks).not.toContain('5.1b')
  })
})
