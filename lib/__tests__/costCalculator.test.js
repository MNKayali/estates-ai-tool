...
// Test report FO-3: a VACANT hospitality unit with a scaffold-licence access
// answer carried "+0.5% for restricted working hours". The rule infers
// restricted hours for trading retail / hospitality premises, but a vacant
// unit has no opening hours to work around.
describe('costCalculator — restricted working hours inferred only for trading premises', () => {
  const hours = cost => cost.trace.prelims.filter(e => /restricted working hours/i.test(e.label))
  const HOSP = { ...BASE, q1_3_buildingUse: 'Hospitality / leisure', q3_5_accessConstraints: ['Scaffold licence or highway encroachment required'] }
  it('does not fire for a vacant unit', async () => {
    expect(hours(await calculateCost({ ...HOSP, q3_6_occupation: 'Vacant or decanted' }, 0))).toEqual([])
  })
  it('fires for an occupied unit, labelled as an assumption', async () => {
    const h = hours(await calculateCost({ ...HOSP, q3_6_occupation: 'Partially occupied' }, 0))
    expect(h).toHaveLength(1)
    expect(h[0].label).toMatch(/assumed for trading hospitality \/ leisure premises/)
  })
  it('a ticked Q3.5 answer fires on any building, with the plain workbook label', async () => {
    const h = hours(await calculateCost({ ...BASE, q3_6_occupation: 'Vacant or decanted', q3_5_accessConstraints: ['Restricted working hours'] }, 0))
    expect(h).toHaveLength(1)
    expect(h[0].label).not.toMatch(/assumed/)
  })
})

// Test reports FO-3, EX-2, EW-3 (BS1) and NB-2 (GL54) priced at the West
// Midlands default: the workbook had no South West region (added in v5.4).
describe('costCalculator — location factor by postcode', () => {
  it.each([['BS1', 'South West'], ['GL54', 'South West'], ['L1', 'North West'], ['SW1A 1AA', 'Inner London'], ['SY10', 'West Midlands']])(
    '%s prices in %s', async (pc, region) => {
      const cost = await calculateCost({ ...BASE, q1_1_postcode: pc }, 0)
      expect(cost.bcisRegion).toBe(region)
      expect(cost.bcisDefaulted).toBe(false)
    })
})
