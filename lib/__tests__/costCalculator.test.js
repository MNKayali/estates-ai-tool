...
describe('cost — Other or mixed rate fallback', () => {
  ...
  it('an item shown for a project type with no rate for it is listed as unpriced, not hidden', async () => {
    // Solar PV is 'Shown on' External works only but has no Ext Works rate.
    ...
    expect(cost.lineItems.some(l => l.code === 'S-0055')).toBe(false)
    expect(cost.excludedNoQuantity.find(e => e.code === 'S-0055')?.reason).toMatch(/no rate/i)
  })
})
