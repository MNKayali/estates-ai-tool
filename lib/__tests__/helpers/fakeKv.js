...
export function createFakeKv() {
  ...
  return {
    ...
    async decr(k) { const n = (Number(store.get(k)) || 0) - 1; store.set(k, n); return n },
    async sadd(k, m) { const s = new Set(store.get(k) || []); s.add(m); store.set(k, [...s]); return 1 },
    async srem(k, m) { store.set(k, (store.get(k) || []).filter(x => x !== m)); return 1 },
    async smembers(k) { return clone(store.get(k) || []) },
    async sismember(k, m) { return (store.get(k) || []).includes(m) ? 1 : 0 },
    async scard(k) { return (store.get(k) || []).length },
    ...
    async hincrby(k, f, n) { const h = { ...(store.get(k) || {}) }; h[f] = (Number(h[f]) || 0) + n; store.set(k, h); return h[f] },
