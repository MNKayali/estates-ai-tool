...
// Batch-2 OM-5 / OM-6: a project past Stage 2 was told to act "before Stage 2 design".
describe('ensureSeedRisks — mitigation wording follows the design stage reached', () => {
  const base = { q3_3_surveys: ['None'], q3_5_accessConstraints: [], q3_1_knownIssues: ['Drainage issues'] }
  const texts = ds => { const p = { riskRegister: [] }; ensureSeedRisks(p, { ...base, q4_5_designStage: ds }); return p.riskRegister.map(r => r.mitigation).join(' | ') }
  it('keeps "Stage 2" for a Stage 0–1 project', () => expect(texts('Concept only (Stage 0–1)')).toMatch(/Stage 2/))
  it('moves to Stage 4 for a developed design, and to tender for a technical design', () => {
    expect(texts('Developed design (Stage 3)')).not.toMatch(/Stage 2/)
    expect(texts('Developed design (Stage 3)')).toMatch(/Stage 4/)
    expect(texts('Technical complete (Stage 4)')).toMatch(/before tender proceeds|pre-tender/)
  })
})
