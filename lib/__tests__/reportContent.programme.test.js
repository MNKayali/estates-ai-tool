...
describe('programme narrative — planning sentence follows the planning status', () => {
  const last = p => programmeNarrativeLines({ stages: [], ...p }).at(-1)
  it('says consent is taken as held, not that no application is needed', () => {
    expect(last({ planningWeeks: 0, planningStatus: 'held' })).toMatch(/taken as already obtained/)
    expect(last({ planningWeeks: 0, planningStatus: 'none' })).toMatch(/No planning application is assumed/)
    expect(last({ planningWeeks: 11, planningAlongside: 'Stage 4' })).toMatch(/holds tender/)
  })
})
