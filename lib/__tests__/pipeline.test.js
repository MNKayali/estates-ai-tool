...
describe('runDeterministicPipeline — one budget story', () => {
  it('every budget sentence the AI and risk register can see matches the final verdict', async () => {
    for (const budget of ['300000', '450000', '600000', '900000']) {
      const { senseCheck } = await runDeterministicPipeline({ ...BASE, q4_3_budget: budget })
      const shortfalls = senseCheck.clientWarnings.filter(w => w.code === 'BUDGET_SHORTFALL')
      if (senseCheck.budget.status === 'insufficient') {
        expect(shortfalls.map(w => w.message)).toEqual([senseCheck.budget.note])
      } else {
