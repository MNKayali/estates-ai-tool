...
import {
  ...
  scopeStatement, coverFactRows, standardsList, buildingUseLabel, specShown, footedRound,
...
describe('specShown — the question, not a stored default (batch-2 EW-4/5/6)', () => {
  it('hides the spec for External works only even when the form stored "Standard"', () => {
    expect(specShown({ q1_2_projectType: 'External works only', q2_4_specLevel: 'Standard' }, { specLevel: 'Standard' })).toBe(false)
    expect(specShown({ q1_2_projectType: 'Demolition only', q2_4_specLevel: 'Standard' }, { specLevel: 'Standard' })).toBe(false)
    expect(specShown({ q1_2_projectType: 'Fit-out', q2_4_specLevel: 'Basic' }, { specLevel: 'Basic' })).toBe(true)
  })
})

describe('works table foots (batch-2 FO-6, RF-5, RF-6; batch-1 DM-1)', () => {
  it('footedRound makes the rounded lines add up to the rounded total', () => {
    // FO-6's high column: 14 lines that rounded one by one summed to £47k against a £45k total.
    const v = [3400, 3400, 3400, 3400, 3400, 3400, 3400, 3400, 3400, 3400, 3400, 3400, 3400, 676]
    const r = footedRound(v, v.reduce((a, b) => a + b, 0), 1000)
    expect(r.reduce((a, b) => a + b, 0)).toBe(45000)
    expect(r.every(x => x % 1000 === 0 && x >= 0)).toBe(true)
  })
  it('worksRows lines foot to the works total in both columns', () => {
    const rows = worksRows(C).filter(r => r.type === 'line')
    const step = C.total.high < 100000 ? 100 : 1000
    expect(rows.reduce((a, r) => a + r.low, 0)).toBe(Math.round(C.works.low / step) * step)
    expect(rows.reduce((a, r) => a + r.high, 0)).toBe(Math.round(C.works.high / step) * step)
    expect(worksGroupRows(C).reduce((a, g) => a + g.low, 0)).toBe(Math.round(C.works.low / step) * step)
  })
})
