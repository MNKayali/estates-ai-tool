...
describe('report — NRM1 v5.2 lines', () => {
  ...
  it('writes quantity basis, unverified-rate flags, BCIS group totals and the below-the-line row into the .docx', async () => {
    ...
    // The report design system (Sept 2026) prints quantity and basis as
    // columns ("30 nr" / "client figure" / "estimated") and flags unverified
    // rates with † on the line.
    expect(text).toContain('30 nr')
    expect(text).toContain('client figure')
    expect(text).toMatch(/[\d,]+ m² estimated/)
    expect(text).toContain('†')
    expect(text).toContain('Other development costs')
    expect(text).toMatch(/5 · SERVICES/i)
