...
function buildProgrammeAssumptions(answers, result) {
  ...
  const bcNote = 'Building control approval: LABC Full Plans route assumed (5–8 weeks). A private RBCA (3–5 weeks) may be considered if programme is critical.'
...
function selectConstructionRow(projectType, specLevel, scopeItems, buildingUse) {
  ...
  // Check by NRM1 code OR legacy text keyword
  const hasLab = scope.some(s => s === '4.4' || s.includes('lab') || s.includes('clinical') || s.includes('healthcare') || s.includes('data centre'))
  const hasHighSpec = specLevel === 'High'
  // GRP 2 codes = fabric/envelope; also check legacy keywords
  const hasFabric = scope.some(s => /^2\.[0-9]/.test(s) || s.includes('roof') || s.includes('facade') || s.includes('window') || s.includes('waterproof'))
  // hasMeOnly = all items are Group 5 M&E (either code or keyword)
  const isMeCode = s => /^5\./.test(s)
  const isMeKeyword = s => s.includes('heating') || s.includes('ventil') || s.includes('electric') || s.includes('plumb') ||
    s.includes('air conditioning') || s.includes('gas') || s.includes('sprinkler') ||
    s.includes('emergency') || s.includes('fire alarm')
  const hasMeOnly = scope.every(s => isMeCode(s) || isMeKeyword(s)) && scope.length > 0
...
export async function calculateProgramme(answers, costMidpoint) {
  ...
  if (bcWks > 0) {
    stages.push({ stage: 'Building Control', activity: hasProgrammePressure ? 'RBCA Full Plans submission' : 'LABC Full Plans submission', weeks: bcWks, notes: 'Stage 3/4 boundary' })
  }
  ...
  const totalWeeks = surveyWeeks + designWeeks + tenderWks + conWks + hoWks + grantGovernanceWks
