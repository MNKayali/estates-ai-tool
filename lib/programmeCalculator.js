...
function selectConstructionRow(projectType, specLevel, scopeItems, storeys) {
  const pt    = projectType.toLowerCase()
  const scope = (scopeItems || []).map(s => s.toLowerCase())
  const hasHighSpec = specLevel === 'High'

  // ── Deterministic project types (scope content doesn't change the row) ───────

  if (pt.includes('new build'))   return 'New Build — Commercial'
  ...
  if (pt.includes('extension')) {
    // Multi-storey if storeys > 1 (requires q1_2_storeys answer)
    return (Number(storeys) > 1) ? 'Extension — Multi-Storey' : 'Extension — Single Storey'
  }

  // ── Scope-driven rows (Refurbishment, Fit-out, Renewable Energy, Mixed) ─────

  // Specialist rooms always override — labs, clinical, data centres
  ...
  // Fit-out: spec level determines Cat A vs Cat B.
  // M&E content is a normal part of a fit-out — do NOT use Refurb rows for fit-out projects.
  if (pt.includes('fit-out') || pt.includes('fit out')) {
    return hasHighSpec ? 'Fit-Out — Cat B / High Spec' : 'Fit-Out — Basic / Cat A'
  }

  // Renewable Energy: typically M&E-driven (heat pumps, PV inverters, battery systems, EV).
  // Let scope analysis decide — a full renewable retrofit is closer to M&E Replacement
  // than to External Works. Falls through to M&E/Fabric checks below.

  // Refurbishment and Mixed: scope analysis picks the most appropriate row
  ...
  // Default: covers Refurb with minor scope, Mixed, Renewable Energy with light scope
  return hasHighSpec ? 'Fit-Out — Cat B / High Spec' : 'Fit-Out — Basic / Cat A'
}
...
export async function calculateProgramme(answers, costMidpoint) {
  ...
  // BC overrun: if approval takes longer than Stage 4, delay carries forward to tender
  ...
  // ── Construction ──────────────────────────────────────────────────────────────
  const conTypeName  = selectConstructionRow(projectType, specLevel, scopeItems, answers.q1_2_storeys)
