...
function selectConstructionId(projectType, specLevel, scopeItems, storeys, buildingUse, interventionLevel) {
...
function buildProgrammeAssumptions(answers, result) {
  ...
  const { tenderWeeks, costMidPoint, occupationUplift, planningRequired, bcRequired } = result
  ...
  return [
    ...
    ...(bcRequired
      ...
      : ['No statutory planning or building control consent is required — the works fall within permitted development. Leasehold or landlord consent (where applicable) is the governing approval and should be secured before works commence.']),
...
export async function calculateProgramme(answers, costMidpoint) {
  ...
  // ── Derived flags ────────────────────────────────────────────────────────────
  // "1945–1980" is caught by its 1945 token; do NOT match a bare "1980", which
  // would also catch "1980–2000" and wrongly flag it as pre-1980 (→ R&D survey).
  const isPreMinus1980 = age.includes('Pre-1900') || age.includes('1900') || age.includes('1945')
  ...
  // ── Design stages ────────────────────────────────────────────────────────────
  const meCount = countMeItems(scopeItems)
  ...
  // ── Construction ──────────────────────────────────────────────────────────────
  const conId       = selectConstructionId(projectType, specLevel, scopeItems, answers.q1_2_storeys, answers.q1_3_buildingUse, answers.q2_3_interventionLevel)
  ...
  milestones.push(`Week ${w}: Stage 3 Developed Design complete${planningRequired ? ' — Planning decision expected' : ''}`)
