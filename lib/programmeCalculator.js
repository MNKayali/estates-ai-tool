...
export async function calculateProgramme(answers, costMidpoint, designMultiplier = 1.0) {
  ...
  // Client gateway — Stage 2 and Stage 4 (all projects)
  const gatewayWks = Math.ceil(dur('Client Gateway', gifa < 150 ? 1 : 2))

  // Stage 3 gateway — 0 for Very Small and Small projects ≤ 250 m² (design proceeds directly to Stage 4)
  const gateway3Wks = gifa < 250 ? 0 : Math.ceil(dur('Client Gateway — Stage 3', 2))
  ...
  // q4_6_phasing: 'Single phase' | 'Multiple phases'
  const phasingAnswer = answers.q4_6_phasing || 'Single phase'
  let phasingNote = ''
  let phasingExtraWks = 0
  if (phasingAnswer.toLowerCase().includes('multiple')) {
    // Two phases assumed at Stage 0–1 — allow mobilisation gap between phases (4 weeks)
    // Phase 2 rebuild will read multiplier from Modifiers PH-1
    phasingExtraWks = conWks + 4
    phasingNote = `Multiple phases: construction programme shown (${conWks} wks) is per phase. Allow an additional ${phasingExtraWks} weeks for subsequent phase(s) including re-mobilisation. Phase split to be confirmed at Stage 2.`
  ...
  // Pre-design surveys run concurrently with Stage 2–3, so surveyWeeks does NOT
  // extend the overall programme duration — they are absorbed within the design period.
  ...
  // ── Milestones ────────────────────────────────────────────────────────────────
  // Stage 2 starts at week 0 (surveys run concurrently, not sequentially before design)
  const milestones = [
    surveyWeeks > 0
