...
function selectConstructionRow(projectType, specLevel, scopeItems) {
  const pt    = projectType.toLowerCase()
  const scope = (scopeItems || []).map(s => s.toLowerCase())

  // Hard-coded project types
  if (pt.includes('new build'))                                  return 'New Build — Commercial'
  if (pt.includes('demolition'))                                 return 'Demolition / Enabling'
  if (pt.includes('external works') || pt.includes('renewable')) return 'External Works'
  ...
  // M&E primary: 3+ M&E items — always the longest row, always conservative
  ...
  // Fabric / envelope — only when M&E is not primary
...
export async function calculateProgramme(answers, costMidpoint) {
  ...
  // ── Construction ──────────────────────────────────────────────────────────────
  const conTypeName  = selectConstructionRow(projectType, specLevel, scopeItems)
